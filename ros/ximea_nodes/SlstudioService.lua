...
local cropBoxStatisticsSetDefaultCropService
local cloudPublisher
local scanActionServer
local heightAnalysisActionServer
local cropBoxStatisticsActionServer
...
local cropBoxStatistics = slstudio_service.CropBoxStatistics()
...
local function handleSlstudioSetup(request, response, header)
  ...
  local code =
    slstudio:initScanStereoThreaded(
    calibrationFile,
    slstudio.CAMERA_ROS,
    leftSerial,
    slstudio.CAMERA_ROS,
...
local function handleHeightAnalysisSetup(request, response, header)
  ...
  ros.INFO(
    'Height Analysis has been configured to mask_min: %f, mask_max: %f',
    heightAnalysisConfig.maskMin,
    heightAnalysisConfig.maskMax
  )
...
local function handleCropBoxStatisticsSetDefaultCropBox(request, response, header)
  cropBoxStatistics:setDefaultCrop(request.crop_box)
  ros.INFO('CropBoxStatistics set default crop box')
  response.success = true
  return true
end
...
function handleNewScanGoal(self)
  ...
  local code, cloud, imageOn, imageOff, imageOnboard, imageShading =
...
function handleNewHeightAnalysisGoal(self)
  ...
  if code == 0 and cloud ~= nil and color_map ~= nil then
    r.success = true
    r.response = ros.Message('ximea_msgs/HeightAnalysisResponse')
    r.height_map = ros.Message('ximea_msgs/HeightMap')
    r.response.image_on = ximea_ros.createImageMessage(imageOn, leftCameraSerial, MONO_COLOR_MODE)
    r.height_map.data = ximea_ros.createImageMessage(height_map, leftCameraSerial, ximea.OPENCV_IMG_FORMAT.CV_32FC1)
    r.response.mask = ximea_ros.createImageMessage(mask, leftCameraSerial, MONO_COLOR_MODE)
    r.response.color_map = ximea_ros.createImageMessage(color_map, leftCameraSerial, RGB_COLOR_MODE)
...
function handleCropBoxStatisticsisGoal(self)
  ...
  if (isScanOnGoing(cropBoxStatisticsActionServer) == true) then
    return
  end

  local goal = cropBoxStatisticsActionServer:acceptNewGoal()

  if (slstudioSetupCorrectly(cropBoxStatisticsActionServer, goal) == false) then
    return
  end
  ...
  if code == 0 and cloud ~= nil then
    r.success = true
    r.number_of_points = point_counts
    cropBoxStatisticsActionServer:setSucceeded(r, 'crop box statistics was successfully.')
    ros.INFO('Scan succeeded.')
    cloud:setHeaderFrameId('world')
    local d = torch.tic()
    cloudPublisher:publish(cloud)
    ...
    r.success = false
    ros.WARN('Scan failed with code %d.', code)
    cropBoxStatisticsActionServer:setAborted(r, 'Scan failed.')
  end
end
...
function SlstudioService:startServices(nh)
  ...
  cropBoxStatisticsSetDefaultCropService = nh:advertiseService('slstudio/setDefaultCropCropBoxStatistics', ros.SrvSpec('ximea_msgs/CropBoxStatisticsSetDefaultCrop'), handleCropBoxStatisticsSetDefaultCropBox)
  ...
  cropBoxStatisticsActionServer = actionlib.SimpleActionServer(nh, 'slstudio/cropBoxStatistics', 'ximea_msgs/CropBoxStatistics')
  cropBoxStatisticsActionServer:registerGoalCallback(function() handleCropBoxStatisticsisGoal(self) end)
  cropBoxStatisticsActionServer:start()
end
...
function SlstudioService:shutdown()
  ...
  if cropBoxStatisticsSetDefaultCropService ~= nil then
    cropBoxStatisticsSetDefaultCropService:shutdonw()
    cropBoxStatisticsSetDefaultCropService = nil
  end
  ...
  if cropBoxStatisticsActionServer ~= nil then
    cropBoxStatisticsActionServer:shutdonw()
    cropBoxStatisticsActionServer = nil
  end
end
